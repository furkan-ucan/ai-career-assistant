...
class SearchQuery:
    ...
    tier: str
    query: str
    metadata: types.MappingProxyType[str, str] = field(default_factory=lambda: types.MappingProxyType({}))
...
class SearchStrategy(ABC):
    ...
    @abstractmethod
    def build_tier1_query(self, persona_data: dict[str, Any]) -> str | None:
        """Build high-precision tier 1 query."""
        pass

    @abstractmethod
    def build_tier2_query(self, persona_data: dict[str, Any]) -> str | None:
        """Build alias/localized tier 2 query."""
        pass

    @abstractmethod
    def build_tier3_query(self, persona_data: dict[str, Any]) -> str | None:
        """Build broad keyword tier 3 query."""
        pass


class BaseSearchStrategy(SearchStrategy):
    """Base class with shared initialization logic for platform-specific strategies."""

    def __init__(self, platform_key: str) -> None:
        try:
            max_or_value: Any = PLATFORM_CONFIGS[platform_key]["max_or_operators"]
            # Type-safe conversion with explicit validation
            if isinstance(max_or_value, int):
                self.max_or_operators: int = max_or_value
            elif isinstance(max_or_value, str):
                self.max_or_operators = int(max_or_value)
            else:
                # Handle any other type by attempting conversion
                # Cast to Any to satisfy mypy's type checking
                self.max_or_operators = int(max_or_value)
        except KeyError as e:
            logger.exception(f"Missing configuration for {platform_key} platform: {e}")
            raise ValueError(f"Missing configuration for {platform_key} platform: {e}")
        except (ValueError, TypeError) as e:
            logger.exception(f"Invalid max_or_operators value for {platform_key}: {e}")
            raise ValueError(f"Invalid max_or_operators value for {platform_key}: {e}")
        self.negative_filters = " ".join(NEGATIVE_FILTERS)


class LinkedInStrategy(BaseSearchStrategy):
    """LinkedIn-specific search strategy following Dr. Finch's LinkedIn Doctrine."""

    def __init__(self) -> None:
        super().__init__(SITE_LINKEDIN)
...
class SearchStrategyFactory:
    ...
    @classmethod
    def build_all_queries(cls, persona_data: dict[str, Any]) -> dict[str, str]:
        """Build all platform-tier query combinations dynamically and safely."""
        queries: dict[str, str] = {}

        for platform in cls._strategies:
            try:
                strategy = cls.get_strategy(platform)
            except Exception as exc:
                logger.exception(f"Failed to initialize strategy for platform '{platform}': {exc}")
                continue
            ...
            try:
                ...
                logger.exception(f"Failed to build queries for platform '{platform}': {exc}")
                continue
