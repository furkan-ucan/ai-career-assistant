    ...
    for persona_name, config in persona_search_config.items():
        ...
        try:
            ...
            if jobs_df_for_persona is not None and not jobs_df_for_persona.empty:
                # Persona bilgisini ve arama terimini ekle (analiz için faydalı)                jobs_df_for_persona['persona_source'] = persona_name
        ...
        if filtered_jobs:            # Uygunluk puanı eşiği ekleme
            ...
            if high_quality_jobs:
                ...
                for i, job in enumerate(high_quality_jobs[:15], 1):  # Top 15 göster
                    ...
                    logger.info(f"   👤 Persona: {job.get('persona_source', job.get('persona', 'N/A'))}")  # Hangi persona aramasıyla geldiği
                ...
                logger.warning(f"⚠️  Filtreleme sonrası {len(filtered_jobs)} ilan bulundu ama uygunluk eşiği (%{MIN_SIMILARITY_THRESHOLD}) altında.")
